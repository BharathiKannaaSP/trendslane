...
import { Card, CardContent, CardFooter, CardTitle } from '@workspace/ui/components/card';
        ...
        {list.map((item) => (
          <Card key={item.id} className='flex-row items-center justify-between gap-4 p-4 '>
            <div className='w-12 h-12 rounded-full relative overflow-hidden'>
              <Image sizes='48px' src={item.image} alt={item.title} fill className='object-cover' />
            </div>
